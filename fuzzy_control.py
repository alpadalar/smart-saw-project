...
def adjust_speeds_based_on_current(processed_speed_data, prev_current, cikis_sim, modbus_client,
                                   ...
                                   speed_adjustment_interval, a_mm):
    ...
    if fuzzy_factor < 0:
        inme_carpan = 0.1
        kesme_carpan = (kesme_orani / 1000)
    else:
        inme_carpan = (kesme_orani / 1000)
        kesme_carpan = 0.1
    ...
    if speed_buffer.adjust_and_check():
        ...
        if ratio < (kesme_orani - 1.0):
            while ratio < (kesme_orani - 1.0):
                if fuzzy_factor < 0:
                    new_serit_kesme_hizi -= 0.1
                elif fuzzy_factor > 0:
                    new_serit_inme_hizi += 0.1
                ratio = (new_serit_inme_hizi / new_serit_kesme_hizi) * 100
        elif ratio > (kesme_orani + 1.0):
            while ratio > (kesme_orani + 1.0):
                if fuzzy_factor < 0:
                    new_serit_inme_hizi -= 0.1
                elif fuzzy_factor > 0:
